...
public class ConnectionHandler
{
	...
	public void get_block_data(int gx, int gy, float[][] currentMesh) {
		...
		int numfloats = LocalWorld.BLOCKS * LocalWorld.BLOCKSIZE; //(((LocalWorld.BLOCKS) * (LocalWorld.BLOCKSIZE - 1)) + 1);
		numfloats *= numfloats;
		...
		for (int x = 0; x < LocalWorld.BLOCKSIZE; x++) {
			for (int y = 0; y < LocalWorld.BLOCKSIZE; y++) {
				sock.Receive(tfloatdata, 4, 0);
				currentMesh[x][y] = (float) BitConverter.ToSingle(tfloatdata, 0);
			}
		}
