...
public class WorldRender : MonoBehaviour {
	...
	private void Init(){
		...
		for (int x = 0; x < BOXES; x++) {
			for (int y = 0; y < BOXES; y++) {
				...
				SquareAndDiamond( xoff, yoff , BOXSIZE , 0.51f); 
				...
				*/
				...
				groundMeshes[x][y] = WeaveMaster(x*BOXSIZE , y*BOXSIZE  , meshbox); 
	...
	private void SquareAndDiamond(int tlx, int tly, int size, float range) {
		...
		float r1 = randGen.GetFloat(0.0f, range);
		float r2 = randGen.GetFloat(0.0f, range * 0.75f);
		range /= 2;
